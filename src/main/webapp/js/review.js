...
console.log("goods review Module ..........................");
...
let reviewService = {
	...
	"list": function(page, callback, error) {
		...
		if (!page) page = 1;
		console.log("page : " + page + ", goods_no : " + goods_no);
		
		// ajax 형태를 만들어 처리합니다. - getJSON()
		$.getJSON( // (url, 성공함수, 실패함수)
			"/goodsReview/list.do?page=" + page + "&goods_no=" + goods_no,
	...
	"write": function(review, callback, error) {
		console.log("댓글 등록 --------------");
		
		$.ajax({
			type: "post", // 데이터 전송 방식
			url: "/goodsReview/write.do", // mapping되는 url을 적는다.
			data: JSON.stringify(review), // 서버에 전송되는 데이터 - body
	...
	"update": function(review, callback, error) {
		console.log("댓글 수정 ----------------------");
		
		$.ajax({
			type : "post", // 데이터 전송 방식
			url : "/goodsReview/update.do", // mapping url -> RestController에 mapping된
			data : JSON.stringify(review), // 서버에 전송되는 데이터
	...
	"delete": function(rno, callback, error) {
		...
		$.ajax({
			type : "get", // 데이터 전송 방식
			url : "/goodsReview/delete.do?rno=" + rno,
			// 성공하는 함수
			success : function(result, status, xhr) {
				if (callback) callback(result);
				else alert(result);
